...
def get_assets():
    ...
    if assets != None:
        response = {
            'message':'Fatched assets Sucessfully.',
            'assets':assets
        }
        return jsonify(response),201
    else:
        response = {
            'message':'Loading assets Failed',
...
def add_transaction():
    ...
    if not values:
        ...
        return jsonify(response),400
    required_fields = ['recipent','productid']
    ...
    signature = wallet.sign_transaction(wallet.public_key,recipent,productid)
    sucess=blockchain.add_transaction(recipent,wallet.public_key,signature,productid)
    if sucess:
        response={
